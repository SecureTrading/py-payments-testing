...
WAITS.register_callable_with_deps('test', Waits, lifetime=InstanceLifetime.Singleton)

REPORTER = NamespacedContainer('reporter')
REPORTER.add_sub_container(CONFIG)
REPORTER.add_sub_container(DRIVER)
REPORTER.register_callable_with_deps('test', Reporter, lifetime=InstanceLifetime.Singleton)
